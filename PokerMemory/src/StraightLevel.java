	...
	protected boolean turnUp(Card card) {
		...
		if(this.getTurnedCardsBuffer().size() < getCardsToTurnUp()) 
		{
			...
			if(this.getTurnedCardsBuffer().size() == getCardsToTurnUp())
			{
				...
				//Verifica que las 5 Cartas levantadas tengan el mismo Suit. Si si, pues resetea el nivel.
				...
				//Si no, pues yeah
				else if ((otherCard.getSuit() != card.getSuit() && otherCard1.getSuit() != card.getSuit()) || (otherCard2.getSuit() != card.getSuit() && otherCard3.getSuit() != card.getSuit()))
					{
